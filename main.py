...
def banner(str):
    ...
    print(f"\t\t\t{red}[ {white}Created By Iruz-Xploit {red}]")
    ...
    while True:
        try:
            ...
            if "BANNED" in str(req_game.text) or req_game.status_code == 403:
                ...
                print(f"{red}[{white}{logTime()}{red}] {white}Nick: {green}{req_game.json()['User']['Username']} {white}| {green}{req_game.json()['User']['Country']} {white}| {green}{req_game.json()['User']['SkillRating']} {white}| {green}{req_game.json()['User']['Crowns']}")
        except:
            continue
